...
const pwaPlugin = (options, context) => {
    ...
    const pwaOptions = Object.keys(options).length > 0 ? options : themeConfig.pwa || {};
    const config = {
        name: "pwa",
        define: () => ({
            PWA_LOCALES: (0, vuepress_shared_1.getLocales)(context, locales_1.pwaLocales, pwaOptions.locales),
            SW_BASE_URL: base || "/",
        }),
        ...
        beforeDevServer(app) {
            app.get(`${base || "/"}manifest.webmanifest`, (_req, res) => {
                (0, genManifest_1.getManifest)(pwaOptions, context)
                    .then((manifest) => {
                    res.send(manifest);
                })
        ...
        ready() {
            context.siteConfig.head = (0, injectHead_1.injectLinkstoHead)(pwaOptions, base, context.siteConfig.head);
        },
        async generated() {
            await (0, genManifest_1.genManifest)(pwaOptions, context);
            await (0, genServiceWorker_1.genServiceWorker)(pwaOptions, context);
        },
    };
