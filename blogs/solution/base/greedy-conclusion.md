...
::: tip update2022/11/13新解法
虽然依然是 多赚/少亏 的思想，但是这种思路会更清晰  
由上面可以看出，我们希望按某两个差值降序排序后，优先选前面的那个  
且在前 $i$ 个里面选 $j$ 个 $x$ ，选 $j$ 个 $y$ 的转移很好想，那么我们初始全选 $a$ ，剩下的就是换数的情况  
至于换成 $b$ 还是换成 $c$ ，就是上面说的优先问题，按 $b-c$ 降序就是优先选 $b$   
说明前面有一部分换也是换成 $b$ ，后面换也是换成 $c$ ，那么就可以枚举分界  
做一个前缀 $dp1[i][j]$ 表示 $[1,i]$ 选 $j$ 个 $b$ 的最大值，$dp2[i][j]$ 表示 $[i,n]$ 选 $j$ 个 $c$ 的最大值  
