...
那么一看就有区间dp那味儿了
如果dp数组两个下标  $i,j$  维护的是选到  $[i,j]$ 时的最大值
那么这个是可以通过转移的，即
$dp[i][j]=max(dp[i-1][j]+a[i]*2^k,dp[i][j+1]+a[j]*2^k)$
$k$ 表示已经选过的个数
...
$4.$从$[i,j]$的  $j$  走向  $i-1$
那么也对应了不同的影响
从  $x$  走向  $y$  这一段产生的影响为 "未关掉的灯泡数量的功率和  $\times$   $x\rightarrow y$ 的路程"
...
状态就记录区间，然后再记录一下当前在区间  $[i,j]$   的左侧  $i$  还是右侧  $j$
那么转移方程为
$dp[i-1][j][0]=min(dp[i][j][1]+walkTime(i-1,j)\times work(i,j))$
$dp[i-1][j][0]=min(dp[i][j][0]+walkTime(i-1,i)\times work(i,j))$
$dp[i][j+1][1]=min(dp[i][j][1]+walkTime(j,j+1)\times work(i,j))$
$dp[i][j+1][1]=min(dp[i][j][0]+walkTime(i,j+1)\times work(i,j))$
