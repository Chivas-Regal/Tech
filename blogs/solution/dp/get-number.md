...
#### 💡
换与不换是一种决策，可以用  $dp$  实现  
换来换去太绕了，我们可以计算直接改变的  $'j'$ 和  $'z'$ 数量，最后计算这两个数量一样的  $dp$ 就行了  
  
那么我们可以记录状态  $dp[i][j][k][id]$ 表示在第  $i$ 位之前，换来了  $j$ 个  $'j'$ 和  $k$ 个  $'z'$ 且这一位是  $id=0?'j':'z'$ ，的最大价值  
  
那么我们线性枚举，枚举第几位，枚举换出了多少个  $'j'$ 和多少个  $'z'$   
**在第  $i$ 位**   

如果这一位本身是  $'j'$ ，我们可以选择换还是不换  
**换：** 产出了一个  $'z'$ ，上一个状态耗费的是  $j-1$ 个换出  $'z'$ 的方式，且可以因上一步最后为  $'j'$ 的位数加价值，或者为  $'z'$ 不改变， $dp[i][j][k][1]=max(dp[i-1][j-1][k][0]+1,dp[i-1][j-1][k][1])$  
**不换：** 本身是  $'j'$ 也不会受到上一步影响，那么就简单汇聚一下， $dp[i][j][k][0]=max(dp[i-1][j][k][0],dp[i-1][j][k][1])$     

如果这一位是  $'z'$   
**换：** 变成了  $'j'$ ，与上一步无影响也耗费了一个  $k$ ，上个状态就是  $k-1$ ，汇聚一下， $dp[i][j][k][0]=max(dp[i-1][j][k-1][0],dp[i-1][j][k-1][1])$   
**不换：**  本身可以和上一步的  $'j'$ 产生影响，  $dp[i][j][k][1]=max(dp[i-1][j][k][1],dp[i-1][j][k][0]+1)$   
  
最后遍历一下修改量  $jk$ ，选择最大的  $max(dp[n][jk][jk][0],dp[n][jk][jk][1])$  
  
其中注意，初始是前面的都弄好了，且这一位不会产生影响，  $dp[0][0][0][1]=0$   
